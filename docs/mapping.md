...
| Gokapi | 1.8 | `infra/k8s/manifests/gokapi.yaml` (no official Helm chart, see that file's header; Tasmane branding via Caddy `html_inject` — `infra/k8s/manifests/caddy-injection.yaml`'s `gokapi-branding.html`, not a Gokapi-side file, see that file's comment for why) |
