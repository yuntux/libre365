...
| Ephemeral staging environment automatically created from IaC | 5.4.1 (L.789) | `.github/workflows/ephemeral-staging.yml` — **simplified**: starts an ephemeral k3d cluster with the production Helm charts (`dev-cluster/deploy.sh`, reusing `infra/k8s/`) plus `docker-compose` for the two bricks with no Kubernetes counterpart (`grommunio-dev`, `novu-mock`), rather than a real Terraform/Ansible-provisioned staging cluster (see the justification in the comment at the top of the file and the "What's missing" section) |
| Representative test dataset (no production data) | 5.4.2 (L.790) | Default `dev-cluster/` + `docker-compose/` configuration (Helm base values, `.env.example`), no connection to production data |
